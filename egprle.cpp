...
void EGPRLE::BitRunDFA(const std::uint8_t* const input, std::size_t length) {
    ...
    while(dfa_pos < length)
    {
        ...
        TARGET(0, 0)
            ...
            GOTO_LOOP;
        TARGET(1, 0)
            buffer.push(dfa_run_length);
            dfa_run_length = 8;
            dfa_state = std::uint8_t{0};
            GOTO_LOOP;
        TARGET(0, 1)
            buffer.push(dfa_run_length + 7);
            dfa_run_length = 1;
