...
void EGPRLE::BitRunDFA(const std::uint8_t* const input, std::size_t length) {
    ...
    std::uint8_t byte = input[dfa_pos];
    std::uint8_t dfa_state = (byte >> 7) & 1;
    ...
    #ifdef COMPUTED_GOTO
        goto *transitions[dfa_state][byte];
    #else
        switch((std::uint16_t{byte} << 1) | dfa_state)
        {
    #endif

    TARGET(0, 0)
        dfa_run_length += 8;
        dfa_state = std::uint8_t{0};
        GOTO_LOOP;
    TARGET(1, 0)
        buffer.push(MOD_PUSH(dfa_run_length));
        dfa_run_length = 8;
        dfa_state = std::uint8_t{0};
        GOTO_LOOP;
    TARGET(0, 1)
        buffer.push(MOD_PUSH(dfa_run_length + 7));
        dfa_run_length = 1;
        dfa_state = std::uint8_t{1};
        GOTO_LOOP;
    TARGET(1, 1)
        buffer.push(MOD_PUSH(dfa_run_length));
        ...
        GOTO_LOOP;
    TARGET(0, 2)
        buffer.push(MOD_PUSH(dfa_run_length + 6));
    ...
    TARGET(1, 3)
        ...
        GOTO_LOOP;
    TARGET(0, 4)
        buffer.push(MOD_PUSH(dfa_run_length + 5));
        buffer.push(MOD_PUSH(1));
        dfa_run_length = 2;
        dfa_state = std::uint8_t{0};
        GOTO_LOOP;
    ...
    TARGET(1, 7)
        ...
        GOTO_LOOP;
    TARGET(0, 8)
        buffer.push(MOD_PUSH(dfa_run_length + 4));
        buffer.push(MOD_PUSH(1));
        dfa_run_length = 3;
        dfa_state = std::uint8_t{0};
        GOTO_LOOP;
    ...
    TARGET(1, 15)
        ...
        GOTO_LOOP;
    TARGET(0, 16)
        buffer.push(MOD_PUSH(dfa_run_length + 3));
        buffer.push(MOD_PUSH(1));
        dfa_run_length = 4;
        dfa_state = std::uint8_t{0};
        GOTO_LOOP;
    ...
    TARGET(1, 31)
        ...
        GOTO_LOOP;
    TARGET(0, 32)
        buffer.push(MOD_PUSH(dfa_run_length + 2));
        buffer.push(MOD_PUSH(1));
        dfa_run_length = 5;
        dfa_state = std::uint8_t{0};
        GOTO_LOOP;
    ...
    TARGET(1, 63)
        ...
        GOTO_LOOP;
    TARGET(0, 64)
        buffer.push(MOD_PUSH(dfa_run_length + 1));
        buffer.push(MOD_PUSH(1));
        dfa_run_length = 6;
        dfa_state = std::uint8_t{0};
        GOTO_LOOP;
